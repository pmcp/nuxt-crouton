...
export async function getAllBookingsSettings(teamId: string, opts: { limit: number; offset?: number }): Promise<{ items: any[]; total: number }>
export async function getAllBookingsSettings(teamId: string, opts?: {}): Promise<any[]>
export async function getAllBookingsSettings(teamId: string, opts: { limit?: number; offset?: number } = {}) {
  const db = useDB()

  const ownerUser = alias(user as any, 'ownerUser')
  const createdByUser = alias(user as any, 'createdByUser')
  const updatedByUser = alias(user as any, 'updatedByUser')
  const conditions = [eq(tables.bookingsSettings.teamId, teamId)]
  ...
  const settings = await listQuery
  ...
  if (opts.limit != null) {
    const [countRow] = await (db as any)
      ...
      .from(tables.bookingsSettings)
      .where(whereExpr)
    return { items: settings, total: Number(countRow?.count ?? 0) }
  }
...
export async function updateBookingsSetting(
  ...
  if (!isAdmin) {
    conditions.push(eq(tables.bookingsSettings.owner, userId))
  }
