...
test.describe(`fixture "${FIXTURE}" i18n`, () => {
  ...
  test('switching locale via the crouton-i18n switcher changes the UI language', async ({ page, baseURL }) => {
    ...
    await expect(page.getByText(before, { exact: false }).first()).toBeVisible({ timeout: 30000 })
    ...
    await expect(page.getByText(after, { exact: false }).first()).toBeVisible({ timeout: 15000 })
