...
import { createShopCategorie, getShopCategoriesByIds } from '../../../../database/queries'
...
export default defineEventHandler(async (event) => {
  ...
  if (dataWithoutId.parentId) {
    const [parent] = await getShopCategoriesByIds(team.id, [dataWithoutId.parentId])
    if (parent) {
  ...
  return await createShopCategorie({
    ...dataWithoutId,
    id: recordId,
    path,
    depth,
