...
//   import { seedContentTestimonials } from '~/layers/content/collections/testimonials/server/database/seed'
//   await seedContentTestimonials({ count: 50, teamId: 'your-team-id' })
//
...
import { contentTestimonials } from './schema'
...
/**
 * Seed testimonials with realistic test data
 */
export async function seedContentTestimonials(options: SeedOptions = {}) {
  const db = options.db ?? createDb()
  const count = options.count ?? 50
  const teamId = options.teamId ?? 'seed-team'

  console.log(`Seeding ${count} testimonials...`)

  if (options.reset) {
    console.log('Resetting testimonials table...')
    await reset(db, { contentTestimonials })
  }

  await seed(db, { contentTestimonials }).refine((f) => ({
    contentTestimonials: {
      count,
      columns: {
        ...
        // Collection fields
        author: f.loremIpsum({ sentencesCount: 1 }),
        company: f.loremIpsum({ sentencesCount: 1 }),
        role: f.loremIpsum({ sentencesCount: 1 }),
        quote: f.loremIpsum({ sentencesCount: 3 }),
        rating: f.int({ minValue: 0, maxValue: 100 }),
