...
async function loadFlowData() {
  try {
    ...
    const [flowResponse, inputsResponse, outputsResponse] = await Promise.all([
      $fetch<Flow>(`/api/teams/${currentTeam.value?.id}/rakim-flows/${flowId.value}`),
      $fetch<FlowInput[]>(`/api/teams/${currentTeam.value?.id}/rakim-flowinputs`),
      $fetch<FlowOutput[]>(`/api/teams/${currentTeam.value?.id}/rakim-flowoutputs`)
    ])
