...
Add real-time collaboration to your Nuxt Crouton applications with the `@friendlyinternet/nuxt-crouton-collab` package. Built on Yjs CRDTs for conflict-free synchronization, it supports rich text editing, flow graphs, and any collaborative data structure.
...
```bash
pnpm add @friendlyinternet/nuxt-crouton-collab
```
