...
Thanks to automatic field detection, you can display lists with minimal setup. Query your collection data and pass it to the `CroutonCollection` component - it will automatically detect and display name, email, and avatar fields.
...
**Deprecated:** The `list-item-actions` slot does not exist in the current source. The examples below are retained for reference but may not work as described. Use the standard actions column or custom Card components instead.
::

The `list-item-actions` slot was intended to customize the action buttons that appear on the right side of each list item, replacing or augmenting the default edit/delete actions.

::callout{icon="i-lucide-info" color="blue"}
**Legacy examples below**: The examples in this section use the deprecated `list-item-actions` slot and are retained for reference only. The current approach is to pass a custom card component via the `card` prop on `CroutonCollection`, or use `CroutonDefaultCard` as a base for customization.
::

### Current Pattern (Recommended)

Use the `card` prop to provide a custom card component that handles its own actions:
...
Then create a `UsersCard.vue` component that extends `CroutonDefaultCard` or builds its own layout with the actions you need. This gives you full control over the card's appearance and actions without relying on deprecated slots.

### Legacy: Basic Custom Actions
...
- [CroutonCollection Component](/api-reference/components#croutonlist) - Complete API reference
