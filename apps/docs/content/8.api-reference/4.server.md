...
```typescript
{
  ...
  member: TeamMember  // The member record proving user is in team
}
```
...
export default defineEventHandler(async (event) => {
  const { team, user, member } = await resolveTeamAndCheckMembership(event)
...
export default defineEventHandler(async (event) => {
  const { team, member } = await resolveTeamAndCheckMembership(event)
...
- **Conditional Authorization**: Skip full member fetch if you only need boolean result
- **Access Verification**: Check team member in API handlers
...
```typescript
export default defineEventHandler(async (event) => {
  const { member } = await resolveTeamAndCheckMembership(event)
