  ...
  await seed(db, { contentArticles }).refine((f) => ({
    contentArticles: {
      ...
      columns: {
        ...
        imageUrl: f.valuesFromArray({ values: ["https://example.com"] }),
        // NOTE: tags references 'tags' - seed tags first, then update this
        tags: f.valuesFromArray({ values: ['placeholder-tags-id'] }),
