...
//   import { seedContentNews } from '~/layers/content/collections/news/server/database/seed'
//   await seedContentNews({ count: 50, teamId: 'your-team-id' })
//
...
import { contentNews } from './schema'
...
/**
 * Seed news with realistic test data
 */
export async function seedContentNews(options: SeedOptions = {}) {
  const db = options.db ?? createDb()
  const count = options.count ?? 25
  const teamId = options.teamId ?? 'seed-team'

  console.log(`Seeding ${count} news...`)

  if (options.reset) {
    console.log('Resetting news table...')
    await reset(db, { contentNews })
  }

  await seed(db, { contentNews }).refine((f) => ({
    contentNews: {
      count,
      columns: {
        ...
        image: f.loremIpsum({ sentencesCount: 1 }),
        text: f.loremIpsum({ sentencesCount: 1 }),
        link: f.valuesFromArray({ values: ["https://example.com"] }),
        status: f.valuesFromArray({ values: ["active", "inactive", "pending"] })
      }
    }
  }))

  console.log(`✓ Seeded ${count} news`)
}
