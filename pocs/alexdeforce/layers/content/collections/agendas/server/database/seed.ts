...
//   import { seedContentAgendas } from '~/layers/content/collections/agendas/server/database/seed'
//   await seedContentAgendas({ count: 50, teamId: 'your-team-id' })
//
...
import { contentAgendas } from './schema'
...
/**
 * Seed agendas with realistic test data
 */
export async function seedContentAgendas(options: SeedOptions = {}) {
  const db = options.db ?? createDb()
  const count = options.count ?? 25
  const teamId = options.teamId ?? 'seed-team'

  console.log(`Seeding ${count} agendas...`)

  if (options.reset) {
    console.log('Resetting agendas table...')
    await reset(db, { contentAgendas })
  }

  await seed(db, { contentAgendas }).refine((f) => ({
    contentAgendas: {
      count,
      columns: {
  ...
  console.log(`✓ Seeded ${count} agendas`)
}
