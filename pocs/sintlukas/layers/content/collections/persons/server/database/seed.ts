...
//   import { seedContentPersons } from '~/layers/content/collections/persons/server/database/seed'
//   await seedContentPersons({ count: 50, teamId: 'your-team-id' })
//
...
import { contentPersons } from './schema'
...
/**
 * Seed persons with realistic test data
 */
export async function seedContentPersons(options: SeedOptions = {}) {
  const db = options.db ?? createDb()
  const count = options.count ?? 25
  const teamId = options.teamId ?? 'seed-team'

  console.log(`Seeding ${count} persons...`)

  if (options.reset) {
    console.log('Resetting persons table...')
    await reset(db, { contentPersons })
  }

  await seed(db, { contentPersons }).refine((f) => ({
    contentPersons: {
      count,
      columns: {
