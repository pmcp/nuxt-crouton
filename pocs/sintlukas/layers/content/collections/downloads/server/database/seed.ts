...
//   import { seedContentDownloads } from '~/layers/content/collections/downloads/server/database/seed'
//   await seedContentDownloads({ count: 50, teamId: 'your-team-id' })
//
...
import { contentDownloads } from './schema'
...
/**
 * Seed downloads with realistic test data
 */
export async function seedContentDownloads(options: SeedOptions = {}) {
  const db = options.db ?? createDb()
  const count = options.count ?? 25
  const teamId = options.teamId ?? 'seed-team'

  console.log(`Seeding ${count} downloads...`)

  if (options.reset) {
    console.log('Resetting downloads table...')
    await reset(db, { contentDownloads })
  }

  await seed(db, { contentDownloads }).refine((f) => ({
    contentDownloads: {
      count,
      columns: {
        ...
        internalName: f.loremIpsum({ sentencesCount: 1 }),
        file: f.loremIpsum({ sentencesCount: 1 })
      }
    }
  }))

  console.log(`✓ Seeded ${count} downloads`)
}
