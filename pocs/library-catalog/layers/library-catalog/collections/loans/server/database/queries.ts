...
export async function getAllLibraryCatalogLoans(teamId: string, opts: { limit: number; offset?: number }): Promise<{ items: any[]; total: number }>
export async function getAllLibraryCatalogLoans(teamId: string, opts?: {}): Promise<any[]>
export async function getAllLibraryCatalogLoans(teamId: string, opts: { limit?: number; offset?: number } = {}) {
  const db = useDB()
