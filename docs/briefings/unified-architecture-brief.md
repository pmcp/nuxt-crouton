...
## Relationship to Crouton Studio

This architecture supports the [Crouton Studio plan](/docs/plans/crouton-studio.md), which is a desktop app for building Nuxt apps through conversation.

### How Studio Uses Both Concepts

```typescript
// apps/studio/nuxt.config.ts
export default defineNuxtConfig({
  // Framework module - provides crouton capabilities
  modules: ['@fyit/crouton'],

  crouton: {
    features: {
      auth: true,
      editor: false  // Studio doesn't need rich text
    }
  },

  // Studio's own domain layer - generated by CLI
  extends: ['./layers/studio']
})
```

Studio's `./layers/studio/` contains:
- `collections/projects/` - Manage user projects
- `collections/artifacts/` - Track generated artifacts
- `collections/chatMessages/` - Chat history
- `collections/generations/` - CLI execution history

**Key Insight**: Studio is both a consumer of the crouton module AND uses generated layers for its own domain. This validates the hybrid architecture.
