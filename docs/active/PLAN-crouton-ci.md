# Crouton CI — Simplified Build Plan
...
|---------|--------|----------|--------------|
| sync-check | ✅ Done | `scripts/validate-field-types-sync.mjs` | Validates field types match across generator, MCP server, and skill file |
| CI workflow | ✅ Done | `.github/workflows/ci.yml` | Lint, typecheck, sync validation, MCP tests |
| TypeScript API | ✅ Auto | `packages/crouton/dist/module.d.ts` | Public API types (generated by unbuild) |
| Changelog data | ✅ Done | `.github/workflows/sync-changelogs.yml` | Daily sync of external package changelogs |
| Audit skill | ✅ Done | `.claude/skills/audit.md` | Manual package auditing via Claude |
...
Source of truth: Generator (packages/nuxt-crouton-cli/lib/utils/helpers.mjs)
                           ↓
Compares against:   ┌─ MCP Server field types (field-types.ts)
                    └─ Claude Skill field types (crouton.md table)
                           ↓
Output:            ✅ All in sync  OR  ❌ Mismatch (exit code 1)
```
...
console.log(`📝 Checking ${packages.length} packages for required docs...\n`)
...
for (const pkg of packages) {
  ...
  if (missing.length > 0) {
    console.log(`❌ ${pkg}: missing ${missing.join(', ')}`)
    hasErrors = true
  } else {
    console.log(`✅ ${pkg}`)
  }
...
**Solution**: Document the convention. No tooling needed — you'll internalize the pattern quickly.
...
❌ pkg.exports["."].types points to non-existent file ./dist/module.d.ts

# attw output when types don't resolve:
❌ "CroutonOptions" is not exported from "@fyit/crouton"
```
...
```
scripts/
├── check-docs.mjs                  # CI: verify CLAUDE.md exists (NEW)
└── validate-field-types-sync.mjs   # CI: verify field types sync (EXISTS)

.github/workflows/
├── ci.yml                          # Lint, typecheck, docs, sync, package (UPDATE)
└── sync-changelogs.yml             # Daily changelog sync (EXISTS)

.claude/skills/
└── audit.md                        # Manual audit via Claude (EXISTS)
...
|------|---------|-----------|
| api-extractor | Track API changes in large libraries | ❌ Overkill |
| publint | Verify package.json exports are valid | ✅ Catches publishing mistakes |
| attw | Verify TypeScript types resolve | ✅ Catches broken types |
...
This means:
- **API stability matters** — external users depend on your types
- **publint/attw catch publishing mistakes** — broken exports or types
- **Breaking changes need marking** — use `!` in commits: `feat(crouton)!: change API`
