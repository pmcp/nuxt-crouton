...
| Phase | Status | Notes |
|-------|--------|-------|
| Phase 0 — Unify (kill `decisions`, single `nodes` collection) | ✅ **SHIPPED** | Migration `0013_phase0_unified_nodes.sql` shipped. `thinkgraph_nodes` is the runtime source of truth. MCP tools updated (`apps/thinkgraph/server/mcp/tools/*-node.ts`). Legacy `decisions/` and `workitems/` collection folders deleted (2026-04-07); the 11 legacy `thinkgraph-decisions/*` endpoints were dead duplicates of the active `thinkgraph-nodes/*` endpoints (no callers anywhere) and were deleted alongside `ThinkgraphWorkitemsNode.vue` (also unreferenced). |
