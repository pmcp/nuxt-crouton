...
- ✅ **Canvas agent activity now visible.** Step 3 rendering layer shipped and live-verified: Pi's `agentStatus`/`agentLog` writes to Yjs (now under `node.ephemeral`, see next entry) render as a pulsing status pill + latest-log-line on the node card (`ThinkgraphNodesNode.vue`) and a full scrollable activity feed in the detail slideover via the new `NodeAgentActivity.vue` component (`apps/thinkgraph/app/pages/admin/[team]/project/[projectId].vue`). The card reads `props.data.ephemeral.{agentStatus,agentLog}` (surfaced from Yjs `node.ephemeral` by `useFlowSyncBridge.ts:172`); the drawer feed opens its own `useFlowSync` against the same flow room while mounted. Auto-scroll uses `nextTick`, no `setTimeout`.
- ✅ **Bridge `data` / `ephemeral` split (Option C) shipped.** Commits `17ce3122` (`packages/crouton-flow`) + `91e61371` (`apps/thinkgraph-worker` + consuming components). `YjsFlowNode` now has a sibling `node.ephemeral` field next to `node.data`; the bridge's row-sync watcher only ever spreads `row` into `node.data`, leaving `node.ephemeral` untouched across refetches — which was the underlying reason Pi's writes were getting clobbered. New `useFlowSync().updateEphemeral(nodeId, patch)` in the package, mirrored by `YjsFlowClient.updateEphemeral` in the Pi worker. Pi's `appendAgentLog`/`setAgentStatus`/`clearAgentState` and the bidirectional control observer (`userPrompt`/`userAbort`/`userSteer`) all moved over. Bridge surfaces the namespace as `data.ephemeral` at the Vue Flow boundary because Vue Flow only forwards `data` to custom node components (the Yjs-level boundary stays honest — `node.ephemeral` is a true sibling of `node.data`). Live-verified during the 2026-04-08 dispatch test against an idle multi-step task — the activity feed populated with status events, tool_use, tool_result, and thinking entries while Pi was working.

**Still open:**

- **Agent activity log feed has three quality bugs (visible during the 2026-04-08 verification).** Step 3 renders, but the data feeding it is noisy: (1) **text events duplicate** — the same complete sentence shows up 2-3 times with the same timestamp, suggesting `session-manager.ts:417` `appendAgentLog` is called once per delta event AND once per final accumulated event for the same model-output chunk; (2) **`tool_result` displays raw JSON** — entries like `{"content":[{"type":"text","text":"..."}]}` instead of the parsed inner text, the formatter at `NodeAgentActivity.vue:91` truncates the JSON string instead of extracting `result.content[].text`; (3) **`thinking` events fragment by token** — single thoughts arrive as separate entries (`...`, `So`, `the`, ...) instead of accumulating into one entry until the thinking block ends. The fix is on the Pi side (session-manager event handling) for (1) and (3), and on the consuming component for (2). Filed as a follow-up, not blocking — the feature works, the data is just ugly.
- **Project page slideover doesn't reactively refresh after dispatch completes.** Pre-existing reactivity bug, surfaced during the 2026-04-08 verification. When Pi finishes a session and posts to `/api/.../dispatch/webhook`, the row in D1 updates (status `working` → `done`, `output` populated, `stage` advances), but the slideover open in the project page does NOT pick this up — `selectedItem` stays stale until the user reloads the tab. Either the page needs to listen on the collection-sync signal that crouton-collab already broadcasts, or `refreshItems()` needs a polling/webhook trigger. Unrelated to Step 3 / Option C, but very visible now that people are watching the slideover during dispatches.
