...
This creates several files in your project: a `List.vue` component for table and list views, a `_Form.vue` component for creating and editing items, a `useShopProducts.ts` composable containing validation, column definitions, and defaults, and a `types.ts` file with TypeScript types.
...
```vue [layers/shop/collections/products/app/components/_Form.vue]
<template>
