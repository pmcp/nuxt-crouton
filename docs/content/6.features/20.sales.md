...
// Kitchen tickets are always generated; the combined customer receipt is
// opt-in via orderOptions.withReceipt (checkout never sets it -- only the
// manual reprint endpoint does; tab clients get theirs via end-receipt).
...
3. The app calls the generic `POST /api/auth/scoped-access/redeem` (resource type `event`) — there is no sales-specific login endpoint. A Nitro plugin in `@fyit/crouton-sales` handles the `crouton:scoped-access:before-redeem` hook and lazily syncs `helperPin` into the event's scoped access grant before verification
...
### One PIN on scoped CMS pages

When a `scoped`-visibility CMS page (from `@fyit/crouton-pages`) embeds the event workspace block, the page's access gate redeems the **event's helper PIN** directly (the server derives the required scope from the block at read time) and the POS panel adopts that session — the volunteer logs in once at the page gate and never sees a second PIN form. Public or hidden pages keep working through the panel's own inline PIN form.
