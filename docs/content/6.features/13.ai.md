...
- `useTranslationSuggestion()` - AI-powered translation suggestions
...
- `AIPageGenerator` - AI-powered page content generation
- `AITranslateButton` - One-click AI translation trigger
...
**Server API Endpoints** (in `/api/ai/`):
- `translate.post.ts` - Translate text content
- `translate-blocks.post.ts` - Translate block-based content
- `generate-email-template.post.ts` - Generate email template content
- `generate-page.post.ts` - Generate page content

::callout{icon="i-lucide-info" color="blue"}
**Note:** The package does not ship a `chat.post.ts` endpoint. The `useChat` composable defaults to `/api/ai/chat`, but you must create this endpoint in your own app (see [Server Usage](#server-usage) for examples).
::
