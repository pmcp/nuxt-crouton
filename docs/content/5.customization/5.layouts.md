...
You can specify which card variant to use for any layout via the `card` prop on `CroutonCollection` or `CroutonCollection`:
...
**✅ DO:**
- Handle both layout types (list, grid) with size variants
- Use the `size` prop to adapt grid cards for different densities
...
- Ignore the layout or size props
...
- Include CroutonItemButtonsMini for consistency
