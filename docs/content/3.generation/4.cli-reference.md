...
::callout{icon="i-lucide-info"}
**Note:** The `add events` command creates the server-side storage layer. To enable automatic event tracking for all collection mutations, also install `@fyit/crouton-events` package. See [Events Package](/features/events) for full documentation.
::
...
- **Package:** [@fyit/crouton-cli](https://www.npmjs.com/package/@fyit/crouton-cli)
- **Repository:** [nuxt-crouton](https://github.com/FriendlyInternet/nuxt-crouton)
