...
> **Note:** `wrangler pages secret put` writes the **Production** secret set only (the
> CLI has no environment flag). If your app has PR/branch **previews**, set
> `BETTER_AUTH_SECRET` on the **Preview** environment too (dashboard, below) — otherwise
> member login fails on previews with `BETTER_AUTH_SECRET is required`.
...
4. Set variables for **Production** — and, if you use PR/branch previews, repeat for the
   **Preview** environment (it has its own separate set; the CLI only writes Production).
   `BETTER_AUTH_SECRET` on Preview can be any 32+ char value — preview sessions are
   independent of production.
5. Click **Save** (env-var changes apply on the *next* deployment — redeploy to pick them up)
