...
**Manifest loader:**
- [ ] Create `packages/crouton-cli/lib/utils/manifest-loader.ts`
- [ ] Implement `discoverManifests()` with monorepo + node_modules scanning
- [ ] Implement `getFieldTypeRegistry()` with alias expansion (flat map)
- [ ] Implement `getAutoGeneratedFields()`, `getReservedFieldNames()`, `getModuleRegistry()`

**CLI integration:**
- [ ] Remove `typeMapping` from `helpers.mjs`, replace with manifest loader
- [ ] Remove `PACKAGE_MANIFESTS` from `manifest-merge.mjs`, replace with manifest loader
- [ ] Update `module-registry.mjs` to use manifest loader
- [ ] Delete `module-registry.json`

**Manifest files:**
- [ ] Fix 5 existing manifests (broken imports, add category/aiHint)
- [ ] Create ~13 new manifests for remaining packages

**consola logging (Wave 2):**
- [ ] Add `consola` to `packages/crouton-cli/package.json`
- [ ] Replace chalk + ora in `helpers.mjs` with consola
- [ ] Replace chalk + ora in `manifest-merge.mjs` with consola
...
- [ ] `field-types.ts` — remove hardcoded types, import from manifest loader
- [ ] `validate-schema.ts` — remove hardcoded auto-generated fields

**Verification:**
- [ ] Run `pnpm crouton doctor .` in test app — CLI reads manifests
- [ ] Run `pnpm crouton config ./crouton.config.js` — generation still works
