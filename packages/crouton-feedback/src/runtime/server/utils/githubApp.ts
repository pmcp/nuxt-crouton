 ...
 *
 * The `github` sink posts its `🎯 Preview feedback` comment as a GitHub App
 * (`*[bot]`) instead of impersonating a person with a standalone PAT. We mint a
 * ~1-hour **installation token** just-in-time: sign an App JWT with the App
 * private key → exchange it for an installation token → use it for the single
 * comment, then let it expire. The one durable secret is the App private key (it
 * never hits the API directly — it only signs JWTs).
 *
 * Done directly with **WebCrypto, dependency-free**, so the package adds nothing
 * to the lockfile and the path runs unchanged on Cloudflare Workers and Node 18+.
 *
 * Ported from @fyit/crouton-devtools (epic #960 / #519).
 */
...
/**
 * Mint a short-lived (~1h) installation token for the App — minted just-in-time,
 * no stored PAT. Throws on a non-2xx mint response; the message is status-only and
 * never echoes the private key or JWT.
 */
export async function mintInstallationToken(
  creds: AppCredentials,
  userAgent = 'crouton-feedback'
): Promise<string> {
