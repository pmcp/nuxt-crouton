...
const OVERLAY_IDS = ['__feedback_launcher_root', '__feedback_annotate_root']

// Client-only singleton state, driven by the registered Annotate tool and read
// by the AnnotateOverlay component.
...
function onClick(e: MouseEvent): void {
  ...
  const file = (el.closest('[data-feedback-src]') as HTMLElement | null)
    ?.getAttribute('data-feedback-src') ?? 'unknown'
  const r = el.getBoundingClientRect()
  panel.value = {
    el,
    file,
...
/**
 * Annotate state + controls. The tool's activate/deactivate map to start/stop;
 * the overlay component renders `hover`/`panel`/`toast`.
 */
