...
/**
 * The unified feedback-tools launcher.
 *
 ...
 * (`useFeedbackTools`); it owns no tool logic. Hidden when no tool is available
 * in the current context.
 *
 * Built entirely on Nuxt UI 4 (UPopover / UButton / USwitch / UIcon). It is
 * mounted into the host app's context by `feedback.client.ts`, so the global
 * U* components resolve.
 */
const { tools, isActive, toggle } = useFeedbackTools()
