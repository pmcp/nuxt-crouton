/**
 * In-process ESC/POS print drainer — Nitro plugin (epic #61).
 *
 ...
 * spooler. This plugin runs that drainer on a poll loop, reusing the same
 * `salesPrintqueues` lifecycle as the spooler (see `escpos-drainer.ts`).
 *
 * OFF by default and on every Cloudflare deploy: it only starts when
 * `CROUTON_SALES_PRINT_DRAINER` is set (the venue Pi sets it), so existing
 * deploys are unchanged and Workers (no raw sockets, no long-lived process)
 * never run it. `node:net` is imported lazily by the drainer, so even loading
 * this plugin is import-safe everywhere.
 *
 *   CROUTON_SALES_PRINT_DRAINER          '1' | 'true' to enable
...
export default defineNitroPlugin(() => {
  const flag = process.env.CROUTON_SALES_PRINT_DRAINER
  if (flag !== '1' && flag !== 'true') return

  const eventId = process.env.CROUTON_SALES_PRINT_DRAINER_EVENT || undefined
  const pollMs = Number(process.env.CROUTON_SALES_PRINT_DRAINER_POLL_MS) || 2000
