...
async function updateRegistry(layer, collection, collectionKey, componentName) {
  const registryPath = path.resolve('crouton.config.ts')
  ...
  try {
    ...
    try {
      ...
      console.log(`↻ Creating crouton.config.ts registry file`)
...
`
    ...
    console.log(`✓ ${fileExists ? 'Updated' : 'Created'} crouton.config.ts with "${collectionKey}" entry`)
  } catch (error) {
