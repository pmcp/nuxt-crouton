...
async function updateRegistry(layer, collection, collectionKey, componentName) {
  const registryPath = path.resolve('app', 'crouton-collections.ts')
  ...
  try {
    ...
    try {
      ...
      console.log(`↻ Creating crouton-collections.ts registry file`)
      content = `// Auto-generated by @friendlyinternet/nuxt-crouton-collection-generator
// Add your collections here or use the generator

export const croutonCollections = {
  // Example: shopProducts: () => import('#imports').then(m => m.useShopProducts),
} as const

export type CroutonCollectionName = keyof typeof croutonCollections
`
    }
    ...
    // Handle both empty and non-empty registries
    if (content.includes('// Example:')) {
      // Replace the example comment with the actual entry
    ...
    console.log(`✓ ${fileExists ? 'Updated' : 'Created'} app/crouton-collections.ts with "${collectionKey}" entry`)
  } catch (error) {
