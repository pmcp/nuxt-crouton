...
/**
 * Liquid Glass Block Editor View
 */
...
const props = defineProps<{
  node: { attrs: LiquidGlassBlockAttrs }
  selected: boolean
  updateAttributes: (attrs: Partial<LiquidGlassBlockAttrs>) => void
  deleteNode: () => void
  getPos: () => number
}>()
  ...
  >
    <div ref="innerRef" class="relative group rounded border border-transparent hover:border-gray-200/50 dark:hover:border-gray-700/50 transition-colors">
      <div class="p-3">
        ...
        <!-- Preview Content -->
        <div class="relative overflow-hidden rounded-lg bg-gradient-to-br from-primary/5 to-primary/10 p-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-base font-bold text-gray-900 dark:text-white">
                {{ attrs.title || 'Glass Card' }}
              </h3>
              <p v-if="attrs.description" class="text-sm text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5">
                {{ attrs.description }}
              </p>
            </div>
          </div>
          <!-- Glass effect indicator -->
          <div class="absolute top-1 right-1 text-[9px] bg-white/60 dark:bg-black/40 backdrop-blur px-1.5 py-0.5 rounded-full text-gray-500">
            glass
          </div>
        </div>
      </div>
