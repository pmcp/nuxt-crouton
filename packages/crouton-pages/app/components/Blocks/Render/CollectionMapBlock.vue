...
/**
 * Auto-detect coordinate field from item data.
 * Looks for fields matching known patterns that contain valid coordinates.
 */
function detectCoordinateField(item: Record<string, any>): string | null {
  // First: check for object-type coordinate fields (e.g. { lat, lng })
  for (const pattern of COORD_FIELD_PATTERNS) {
    for (const key of Object.keys(item)) {
      if (key.toLowerCase() === pattern.toLowerCase()) {
        ...
        // Array [lng, lat] (GeoJSON order)
        if (Array.isArray(value) && value.length === 2 && typeof value[0] === 'number') {
          return key
        }
        // String that looks like coordinates "lat,lng"
...
/**
 * Extract [lng, lat] from a coordinate field value.
 */
...
// Resolved coordinate field name
...
// Markers: items with valid coordinates
...
// Default center when there are no markers
...
<template>
  <div class="collection-map-block my-8">
    ...
    <UAlert
      ...
      description="Add @fyit/crouton-maps to your app to enable collection map blocks."
    />
