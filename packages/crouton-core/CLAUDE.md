...
| `app/composables/useCroutonLayoutBlocks.ts` | **Layout block registry** reader (#704) — reads `app.config.croutonLayoutBlocks`; `getBlock`/`hasBlock`/`resolveComponentName` (allowlisted id → component **NAME**) + `sanitizeConfig` (validate per-block config vs the block's `configSchema`). Pure `resolveLayoutBlockComponentName` / `sanitizeLayoutBlockConfig` exported for tests. Also `checkViability(tree, targetWidths)` (#710) — viability bound to the live registry — and `composeDefault(collections, opts?)` (#709) — the deterministic default layout bound to the live registry (same rule set the CLI runs at generate time). **Distinct** from the TipTap `croutonBlocks` content-block registry — layout blocks are pane-placeable surfaces. |
| `app/types/layout-block.ts` | `CroutonLayoutBlockDefinition` (id/name/description/icon + component **string** + `kind`/`configSchema`) — the placeable-block contract (mirrors `CroutonPageType`). Plus the **sizing contract** (#710): `minWidth`/`minHeight`/`maxWidth`/`defaultSize`/`aspect`/`resize`/`density` — all optional; undeclared = fully fluid (`minWidth` 0). |
| `app/types/layout.ts` | `LayoutTree` / `LayoutNode` (`split` \| `leaf`) — the persisted "layout is data" tree. |
| `app/components/LayoutRenderer.vue` | `CroutonLayoutRenderer` — recursive **read-only** renderer of a layout tree into **reka-ui Splitter** panes; resolves each leaf `blockId` via the registry (unknown id → safe fallback, never an arbitrary component). SSR-safe (no `<ClientOnly>`). Enforces **runtime min-width** (#706): measures each group's live width (`useElementSize`) and converts each block's px floor to the panel `min-size` % via `panelMinSizePct`. Used by `paneBlock` public render. |
| `app/components/Layout.vue` | `CroutonLayout` (#706) — the **editable** panes surface (new; distinct from the read-only renderer). Palette of `croutonLayoutBlocks` (draggable) + editable canvas + per-block config panel (from `configSchema`) + live viability badge + preview toggle. `v-model` is the `LayoutTree` (`LayoutTree \| null`; null = empty). Every edit is a pure `layout-edit` transform; emits the new tree for the owner to persist. |
| `app/components/LayoutEditorPane.vue` | `CroutonLayoutEditorPane` (#706) — recursive **editable** pane (the editor counterpart to LayoutRenderer): HTML5 drop zones (edge = split/nest, center = swap), per-pane toolbar (configure/remove), selection highlight, same allowlisted leaf preview + min-width enforcement. Delegates edits to the provided `LayoutEditApi`. |
| `app/composables/useCroutonLayoutEdit.ts` | `LayoutEditApi` + `LAYOUT_EDIT_KEY` (#706) — the provide/inject edit contract (`drop`/`remove`/`resize`/`select` keyed by a `NodePath`, plus `dragging`/`selectedPath` refs) that `CroutonLayout` gives every editor pane. `pathKey`/`parsePathKey` helpers. |
| `app/composables/useCroutonLayoutStore.ts` | Load/save a `LayoutTree` to the team-scoped `layout_configs` table (#706, production successor to the spike store). `load` re-runs `sanitizeLayoutTree` (untrusted on the way out too); `save` is debounced. |
| `app/utils/layout-edit.ts` | Pure layout-tree edit transforms (#706) — `dropBlock` / `splitLeaf` / `removeNode` (collapses a split to its survivor) / `applySizes` / `setConfig`, addressed by a `NodePath`. Immutable (return a new root). The compose loop's testable core. |
| `app/types/block-core.ts` | `CroutonBlockCore` (#716) — the surface-agnostic projection both block registries share (id/name/description/icon/category + `renderer` NAME + `clientOnly` + `surface`). A read-side projection, **not** a base both definitions `extends` (keeps the working TipTap `croutonBlocks` shape — `type`/`components.renderer` — untouched). |
| `app/composables/useCroutonBlockCatalog.ts` | **Unified block catalog** (#716) — one read facade over BOTH registries (`croutonBlocks` TipTap content + `croutonLayoutBlocks` panes). `getBlock(id)` / `listBlocks(surface?)` answer from a single merged `id → CroutonBlockCore` map (pane wins an id collision). Pure `documentBlockToCore` / `layoutBlockToCore` / `buildBlockCatalog` exported for tests. So pages/panes/(later)flow ask one place. |
| `app/utils/layout-tree.ts` | `sanitizeLayoutTree(input)` (#716) — pure shape gate for an **untrusted** `LayoutTree` (page content). Returns a clean tree (known fields only — drops stray keys / prototype pollution, depth-capped) or `null` → caller renders a fallback. Does NOT resolve components/config (that stays the renderer + `sanitizeConfig` allowlist). Accepts a full `{ renderer, root }` or a bare root node. |
| `app/utils/layout-viability.ts` | **Viability metric** (#710) — the objective gate the layout engine leans on. `checkLayoutViability(root, minWidthFor, targetWidths)` / `checkTreeViability(tree, …)` → `{ viable, violations[] }`: a layout is viable iff every placed block gets ≥ its `minWidth` (px) in its pane at the target width(s). Pure, width-only (horizontal split divides width by `defaultSize` share; vertical keeps full width). `minWidthResolver(registry)` derives the resolver; `useCroutonLayoutBlocks().checkViability(tree, widths)` binds it to the live registry. Also `subtreeMinWidth(node, resolver)` (horizontal sums children, vertical maxes) and `panelMinSizePct(...)` (px floor → SplitterPanel `min-size` %) — the **runtime** enforcement (#706) the renderer/editor use. Reused by the deterministic layout (#709). |
| `app/utils/layout-compose.ts` | **Deterministic default-layout pass** (#709) — `composeDefaultLayout({ collections, registry, targetWidths?, blockIds? })` → `{ tree, pattern, viable, violations }`. The legible rule set that arranges generated collections into a good default: a compound **calendar** present → **calendar-primary**; list + form → **master-detail**; only a form → **form-centric**; extra collections stacked vertically. Each leaf is data-bound (`config.collection`). Gated by `checkLayoutViability` with a **vertical fallback** (a too-narrow horizontal split becomes a vertical stack, which keeps full width → viable). Pure, so the SAME rules run in the CLI generate → POC pipeline (exported at `@fyit/crouton-core/app/utils/layout-compose`) and in the Nuxt app via `useCroutonLayoutBlocks().composeDefault(...)`. The LLM pass (#711) is out of scope. |
| `app/components/LayoutCollection.vue` + `LayoutCollectionData.vue` | `CroutonLayoutCollection` (#709) — a **real data-bound list block** (replaces `LayoutSpikeList`). Renders an actual generated collection's rows by name via `CroutonCollection` (`config.collection`). Container-responsive (`@container`); tolerates a missing `collection` (freshly dropped block) by showing a hint. The async `useCollectionQuery` lives in the inner `…Data` component behind `<Suspense>` so an unconfigured block never fetches. |
| `app/components/LayoutForm.vue` | `CroutonLayoutForm` (#709) — a **real data-bound form block** (replaces `LayoutSpikeForm`). The "detail" half of master/detail: an always-visible inline create form for `config.collection` via `CroutonFormDynamicLoader` (resolves the generated `{Name}Form`). Container-responsive; tolerant of a missing `collection`. |
| `app/components/LayoutSpikeStats.vue` | `CroutonLayoutSpikeStats` — the remaining generic (non-collection) KPI demo block, registered as `stats`. The data-bound `LayoutSpike{List,Form}` placeholders were replaced by the real blocks above at #709. |
| `app/pages/admin/[team]/layout.vue` | `CroutonLayout` harness (#706) — loads/edits/saves a team layout via `useCroutonLayoutStore`; the editable-surface route. Replaced the spike harness. |
| `server/database/schema/layoutConfigs.ts` + `server/api/teams/[id]/crouton-layouts/[layoutId].{get,put}.ts` | `layout_configs` table (team-scoped JSON layout trees) + CRUD. The PUT validates the untrusted tree with `sanitizeLayoutTree` server-side before storing (#706). |
