...
| `app/types/layout.ts` | `LayoutTree` / `LayoutNode` (`split` \| `leaf` \| `nested`) — the persisted "layout is data" tree. A `nested` node hosts a whole sub-layout (`{ type:'nested', layout: LayoutTree, label? }`) so a page-layout's panes can be app-layouts (recursive "layouts in layouts", WS2 #871). Plus `LayoutBreakpoint` + `LayoutTree.breakpoints?` (WS5 #874) — authored min-width checkpoints that **lock upward** (per-field last-wins: `root` arrangement / `collapsed` set / `variants` / `collapseStyle`); resolved by `crouton-layout`'s `resolveLayoutAtWidth`. |
