...
| _Spike blocks (still registered until Sprint 4 #709):_ `app/components/LayoutSpike{List,Form,Stats}.vue` | Self-contained, container-responsive demo blocks the palette places (`croutonLayoutBlocks` in `app/app.config.ts`); real data-bound list/form blocks replace them in Sprint 4. The spike harness page + store were promoted to `layout.vue` + `useCroutonLayoutStore` at #706. |
