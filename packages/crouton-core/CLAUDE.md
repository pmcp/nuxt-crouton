...
| `app/types/layout.ts` | `LayoutTree` / `LayoutNode` (`split` \| `leaf` \| `nested`) — the persisted "layout is data" tree. A `nested` node hosts a whole sub-layout (`{ type:'nested', layout: LayoutTree, label? }`) so a page-layout's panes can be app-layouts (recursive "layouts in layouts", WS2 #871). |
