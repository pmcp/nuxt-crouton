 ...
 */
export async function resolveAppConfigPath(cwd = process.cwd()) {
 ...
 */
function getAppConfigOptions(mod) {
 ...
 * @param {string} registryPath - Path to app.config.ts
 * @param {string} collectionKey - Key in croutonCollections object
 * @param {string} configName - Imported config variable name
 * @param {string} importSource - Module path to import from
 * @returns {Promise<{added: boolean, created?: boolean, reason?: string}>}
 */
export async function addToAppConfig(registryPath, collectionKey, configName, importSource) {
...
`
  ...
  try {
    ...
    const hasImport = mod.imports.$items.some(
      item => item.local === configName || item.imported === configName
    )
    if (!hasImport) {
      mod.imports.$append({
        imported: configName,
 ...
 */
export async function removeFromAppConfig(registryPath, collectionKey, configName) {
 ...
 */
export async function registerTranslationsUiCollection(cwd = process.cwd()) {
