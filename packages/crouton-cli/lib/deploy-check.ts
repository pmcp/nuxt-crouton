...
function checkWranglerConfig(config: AppConfig): DeployCheck[] {
  ...
  if (placeholders.length > 0) {
    checks.push({
      name: 'wrangler-ids',
      status: 'fail',
      message: `Placeholder IDs found: ${placeholders.join(', ')} — run \`crouton deploy-setup\` or set manually`,
    })
  } else {
    checks.push({
      name: 'wrangler-ids',
...
export function printDeployReport(result: DeployCheckResult): void {
  ...
  if (fails > 0) {
    ...
    console.log()
    console.log('  Fix blockers before deploying. Run `crouton deploy-setup` to auto-generate missing config.')
  } else if (warns > 0) {
