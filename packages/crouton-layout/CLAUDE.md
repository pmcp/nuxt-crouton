...
| `app/components/LayoutResponsiveRenderer.vue` | `CroutonLayoutResponsiveRenderer` — renders a tree **with breakpoints applied**: resolves at the measured (or simulated `width`) container width, provides variants. **Branches on the resolved collapse style (WS6 #875):** `gutter-tabs` → draws the visible tree minus collapsed panes + a right gutter rail; an *in-place* style → renders the FULL resolved tree (collapsed panes stay in their slots as handles) and provides `LAYOUT_COLLAPSE_KEY`. The explicit layer atop `CroutonLayoutRenderer`. **Bubbles `layoutChange` (#874 follow-up):** when a splitter is dragged, it resolves the resized split *node* → its `NodePath` within the resolved root and emits `(path, sizes)`, so an author resolving the same tree at the same width can apply the sizes onto a structurally-identical root. |
