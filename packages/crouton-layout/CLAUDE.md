...
| `app/utils/layout-viability.ts` | **Viability metric** (#710) — `checkLayoutViability` / `checkTreeViability` → every placed block gets ≥ its `minWidth` in its pane. `minWidthResolver` / `panelMinSizePct` (runtime min-size enforcement). `isCollapsed` (#852) — a collapsed pane leaves the splitter, so the metric + the runtime floor skip it (and it stops constraining siblings); re-opening restores enforcement. |
| `app/utils/layout-edit.ts` | Pure layout-tree edit transforms (#706) — `dropBlock` / `splitLeaf` / `removeNode` / `applySizes` / `setConfig` / `setOpen` (#852, persists a collapsible pane's open-state), addressed by a `NodePath`. Immutable. |
...
| `app/composables/useCroutonLayoutStore.ts` | Load/save a `LayoutTree` to the team-scoped `layout_configs` table (re-sanitizes via `sanitizeLayoutTree`; debounced save). Per-pane collapse state (#852) rides the tree's nodes, so it persists/restores with the same save — no localStorage. |
...
| `app/components/LayoutRenderer.vue` | `CroutonLayoutRenderer` — recursive **read-only** renderer into reka-ui Splitter panes; resolves each leaf `blockId` via the registry. **Collapsible panes** (#852): a `collapsible` child closes into its `collapse` affordance — the reference `gutter-tabs` (lifted from sales `Shell.vue`) drops it from the splitter to a vertical tab in a reserved right gutter (`pe-11`), with the open pane carrying a ✕ to re-collapse. Toggling emits `openChange(path, open)` (persist via `setOpen` → `useCroutonLayoutStore`) and keeps interactive local state for read-only trees (demo / `paneBlock`). |
