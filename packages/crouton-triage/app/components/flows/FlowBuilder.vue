...
import type { Flow, FlowInput, FlowOutput, NotionOutputConfig, NotionInputConfig } from '~/layers/triage/types'
...
async function suggestPersonalityIcons() {
  ...
  try {
    const response = await $fetch<{ suggestions: IconSuggestion[] }>(`/api/crouton-triage/teams/${props.teamId}/ai/suggest-icons`, {
      method: 'POST',
      body: {
...
const notionWebhookUrl = computed(() => {
  if (typeof window !== 'undefined') {
    return `${window.location.origin}/api/crouton-triage/webhooks/notion-input`
  }
  return '/api/crouton-triage/webhooks/notion-input'
})
...
async function testNotionConnection() {
  ...
  try {
    ...
    const response = await $fetch(`/api/crouton-triage/teams/${props.teamId}/notion/test-connection`, {
      method: 'POST',
      body: {
...
function resetInputForm(sourceType: 'slack' | 'figma' | 'email' | 'notion') {
  ...
  if (sourceType === 'notion') {
    ...
    inputFormState.triggerKeyword = 'triage'
  } else {
    inputFormState.notionToken = ''
...
async function saveInput(event: FormSubmitEvent<InputFormData>, close: () => void) {
  ...
  if (event.data.sourceType === 'notion') {
    sourceMetadata = {
      ...sourceMetadata,
      notionToken: inputFormState.notionToken,
      triggerKeyword: inputFormState.triggerKeyword || 'triage'
  ...
  if (savedFlowId.value) {
    try {
      const savedInput = await $fetch<FlowInput>(`/api/teams/${props.teamId}/triage-flowinputs`, {
        method: 'POST',
        body: {
          ...inputData,
...
async function deleteInput(index: number) {
  ...
  if (input.id) {
    try {
      await $fetch(`/api/teams/${props.teamId}/triage-flowinputs/${input.id}`, {
        method: 'DELETE',
      })
    } catch (error: any) {
      toast.add({
...
const { fetchNotionSchema, schema: notionSchema, loading: fetchingSchema, error: schemaError } = useTriageNotionSchema()
const { generateAutoMapping, getPropertyTypeColor } = useTriageFieldMapping()
...
async function deleteOutput(index: number) {
  ...
  if (output.id) {
    try {
      await $fetch(`/api/teams/${props.teamId}/triage-flowoutputs/${output.id}`, {
        method: 'DELETE',
      })
    } catch (error: any) {
      toast.add({
...
async function onFlowSubmit(event: FormSubmitEvent<FlowSchema>) {
  ...
  if (!savedFlowId.value) {
    ...
    try {
      ...
      const flowResponse = await $fetch<{ id: string }>(`/api/teams/${props.teamId}/triage-flows`, {
        method: 'POST',
        body: flowData
      })
...
async function saveFlow() {
  ...
  try {
    ...
    await $fetch(`/api/teams/${props.teamId}/triage-flows/${flowId}`, {
      method: 'PATCH',
      body: flowData
    })
    ...
    for (const input of inputsList.value) {
      ...
      if (!input.id) {
        try {
          const savedInput = await $fetch(`/api/teams/${props.teamId}/triage-flowinputs`, {
            method: 'POST',
            body: {
              ...input,
              flowId
    ...
    for (const output of outputsList.value) {
      if (!output.id) {
        await $fetch(`/api/teams/${props.teamId}/triage-flowoutputs`, {
          method: 'POST',
          body: {
            ...output,
            flowId
                ...
                <div class="p-6">
                  ...
                  <div v-if="editingInput" class="space-y-4">
                    ...
                    <UFormField
                      v-if="editingInput.sourceType === 'notion'"
                      label="Trigger Keyword"
                      help="Keyword that triggers task creation (e.g., triage)"
                    >
                      <UInput
