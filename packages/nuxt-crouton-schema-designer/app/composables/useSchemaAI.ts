...
2. **NEVER include timestamp or audit fields** - The following are AUTO-GENERATED by the CLI and must NOT be in your schema:
   - createdAt, updatedAt (timestamps)
   - createdBy, updatedBy (user references)
   - Do NOT ask if the user wants these fields - they are always added automatically
3. Use **camelCase** for field names (e.g., userId, productName)
4. Choose the most appropriate field type for the data:
...
5. Mark important fields as required: true
6. Set appropriate maxLength for string fields
7. If the user wants to modify existing fields, output the FULL schema with all fields (existing + modified)
8. When adding fields to an existing schema, output ALL fields including the existing ones
${currentStateJson}
