...
export function useSchemaValidation(collections: Ref<CollectionWithFields[]>) {
  const issues = computed<ValidationIssue[]>(() => {
    ...
    const collectionNames = new Map<string, string>() // name → id
    ...
    for (const col of collections.value) {
      ...
      if (RESERVED_COLLECTION_NAMES.has(lowerName)) {
        result.push({
          type: 'error',
          collectionId: col.id,
          message: `"${col.name}" is a reserved name`,
      ...
      for (const field of col.fields) {
        ...
        if (fieldNames.has(lowerFieldName)) {
          result.push({
            ...
            collectionId: col.id,
            fieldId: field.id,
            message: `Duplicate field name "${field.name}" in "${col.name}"`,
        ...
        if (RESERVED_NAMES.has(field.name)) {
          result.push({
            ...
            collectionId: col.id,
            fieldId: field.id,
            message: `"${field.name}" is auto-generated by Crouton — consider renaming`,
        ...
        if (field.type === 'reference' && !field.refTarget) {
          result.push({
            ...
            collectionId: col.id,
            fieldId: field.id,
            message: `Reference field "${field.name}" has no target collection`,
        ...
        if (field.type === 'reference' && field.refTarget) {
          ...
          if (!targetExists) {
            result.push({
              ...
              collectionId: col.id,
              fieldId: field.id,
              message: `Reference target "${field.refTarget}" does not exist`,
        ...
        if (meta.dependsOn) {
          ...
          if (!parentExists) {
            result.push({
              ...
              collectionId: col.id,
              fieldId: field.id,
              message: `dependsOn field "${meta.dependsOn}" does not exist in "${col.name}"`,
        ...
        if (meta.dependsOnCollection) {
          ...
          if (!targetCol) {
            result.push({
              ...
              collectionId: col.id,
              fieldId: field.id,
              message: `dependsOnCollection "${meta.dependsOnCollection}" does not exist`,
            ...
            if (!targetField) {
              result.push({
                ...
                collectionId: col.id,
                fieldId: field.id,
                message: `dependsOnField "${meta.dependsOnField}" does not exist in "${meta.dependsOnCollection}"`,
    ...
    const circularRefs = detectCircularReferences(collections.value)
...
function detectCircularReferences(collections: CollectionWithFields[]): ValidationIssue[] {
