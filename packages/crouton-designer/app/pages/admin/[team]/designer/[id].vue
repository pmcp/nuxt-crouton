...
// Detect whether the Phase 2 system prompt (with full field type tables) has already been sent
// to the AI in a prior turn. Checks for any assistant message after the last phase marker.
// When true, the prompt builder sends a compact reference instead of the full tables (~1400 token saving).
const hasPriorPhase2Context = computed(() => {
  if (currentPhase.value !== '2') return false
  // Find the last phase-transition marker (system messages inserted by insertPhaseMarker)
  let markerIdx = -1
