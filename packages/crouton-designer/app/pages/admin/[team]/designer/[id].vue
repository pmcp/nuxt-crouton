...
// Detect whether the current phase's system prompt has already been sent in a prior turn.
// Checks for any assistant message after the last phase marker (system msg starting with '---').
// Used by prompt builders to skip verbose context that's already in the model's history.
function hasPriorContextForPhase(phase: string): boolean {
  let markerIdx = -1
  for (let i = messages.value.length - 1; i >= 0; i--) {
    const msg = messages.value[i] as any
    if (msg.role === 'system' && typeof msg.content === 'string' && msg.content.startsWith('---')) {
      markerIdx = i
      break
    }
  }
  if (markerIdx === -1) return false
  return messages.value.slice(markerIdx + 1).some(m => m.role === 'assistant')
}

const hasPriorPhase2Context = computed(() =>
  currentPhase.value === '2' && hasPriorContextForPhase('2')
)
const hasPriorPhase3Context = computed(() =>
  currentPhase.value === '3' && hasPriorContextForPhase('3')
)
...
const systemPrompt = computed(() => {
  ...
  if (currentPhase.value === '2') {
    ...
    return buildCollectionPrompt(projectConfig.value, collections, hasPriorPhase2Context.value)
  }
