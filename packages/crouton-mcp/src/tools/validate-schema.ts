...
import { z } from 'zod'
import { isValidFieldType, VALID_FIELD_TYPES, getAutoGeneratedFields } from '../utils/field-types.js'
...
export function handleValidateSchema(input: ValidateSchemaInput): ValidationResult {
  ...
  for (const [fieldName, field] of Object.entries(schema)) {
    // Silently skip auto-generated fields (from manifests)
    const autoGenerated = getAutoGeneratedFields()
    if (autoGenerated.includes(fieldName)) {
      continue
    }
  ...
  // Check for at least one meaningful field (not just auto-generated ones)
  const allAutoGenerated = getAutoGeneratedFields()
  const meaningfulFields = Object.keys(schema).filter(
    f => !allAutoGenerated.includes(f)
  )
