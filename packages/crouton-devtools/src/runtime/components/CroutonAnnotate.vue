...
/**
 * Annotate overlay (#810) — the highlight outline + comment panel for the
 * Annotate tool. Rebuilt on Nuxt UI 4 (UTextarea/UButton), replacing the old
 * vanilla-DOM review overlay; the pure capture helpers (overlay/capture.ts) are
 * reused unchanged. Mounted into the host app's context by
 * `plugins/tools/annotate.client.ts`.
 */
const { selecting, hover, panel, sending, toast, closePanel, send } = useCroutonAnnotate()
