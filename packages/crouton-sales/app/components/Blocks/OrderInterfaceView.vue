...
/**
 * Inline POS Block Editor View
 *
 * NodeView component for rendering the inline POS block inside the page
 * editor. Like the storefront block: shows a placeholder + edit/delete
 * controls, opens property panel on double-click.
 *
...
const props = defineProps<{
  node: { attrs: OrderInterfaceAttrs }
  selected: boolean
  updateAttributes: (attrs: Partial<OrderInterfaceAttrs>) => void
  deleteNode: () => void
  getPos: () => number
}>()
...
<template>
  <NodeViewWrapper
    ...
    data-type="order-interface-block"
  ...
  >
    <div ref="innerRef" class="relative group rounded border border-transparent hover:border-gray-200/50 dark:hover:border-gray-700/50 transition-colors">
      <div class="p-3">
        ...
        <!-- Mini POS preview: category tabs over a product grid -->
        <div class="bg-gray-50/50 dark:bg-gray-800/30 rounded-lg p-4 border border-gray-100 dark:border-gray-700/50">
          <div class="flex gap-1 mb-3">
            <div v-for="t in 3" :key="t" class="h-3 w-12 rounded bg-gray-200/60 dark:bg-gray-700/40" />
          </div>
          <div class="grid grid-cols-4 gap-2">
            <div v-for="p in 8" :key="p" class="aspect-square rounded bg-gray-200/60 dark:bg-gray-700/40" />
          </div>
        </div>
      </div>
    </div>
