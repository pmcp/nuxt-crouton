...
|-----------|------------------|---------|
| `Cart.vue` | `SalesClientCart` | Shopping cart display with quantity controls. Line items (`OrderLineItem.vue`) top-align price + qty controls with the product name and stack **option labels and the per-item remark** (warning-colored) underneath, each with a small bullet, no indentation; products are separated by a subtle `divide-y divide-default/60` divider (same as the workspace's expanded order). Footer carries a **Remark section**: one elevated block (`UCollapsible` wrapper with `bg-elevated/60`, ghost toggle button so it reads as a unit — taller `py-3`, and while open it squares its bottom corners + draws a `border-b` separator above the accordion; count badge when collapsed; **plus/minus trailing icon** — deliberately not a chevron, the accordion rows own the chevrons; hidden when no cart item has a location) containing a **`UAccordion` (`type="multiple"`) with one item per location that has items in the cart** — each body is that location's textarea (→ `locationRemarks`, printed as `REMARK:` on that location's ticket only; `variant="soft"` + `text-toned font-normal` triggers keep the rows light), and a filled textarea shows a primary dot next to the accordion label. The cart card itself trims UCard's default side padding to `px-3`. **There is no whole-order remark UI anymore** — `usePosOrder().overallRemarks` still exists and checkout still sends it (always `null` from the POS), and OrdersTab still displays historical `overallRemarks`. Footer also has a **Staff order** `USwitch` (`isPersonnel`) for the staff banner. Remarks + staff flag bind to `usePosOrder()` refs via props + emits in OrderInterface. Cart items snapshot the product at add-time; OrderInterface watches its products source and calls `usePosOrder().syncCartProducts(fresh)` so admin edits (location/price/title) reach items already in the cart |
