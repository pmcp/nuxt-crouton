...
| Skill | `.claude/skills/ticket-diagram/SKILL.md` | Attach an editable **Excalidraw** status diagram to a GitHub epic — read the epic + sub-issue tree → boxes-coloured-by-status with bound dependency arrows → committed `.excalidraw` (editable source) + a **PNG that renders on the GitHub mobile app** (offline render via the shared `ui-proposal` headless-Chromium pipeline) → sticky comment → iterate to approval (reuses the #310 sign-off loop). Generator: `scripts/ticket-excalidraw.mjs` (+ `scripts/lib/excalidraw.mjs`). NOT live Mermaid (stalls on mobile). Workstream #2 of #479, epic #483 |
