...
| Skill | `.claude/skills/mockup/SKILL.md` | Explore an idea as throwaway, **clickable** no-build mockups *before* a working prototype — the lighter, earlier sibling of `/ui-proposal`. Scaffolds `mockups/<slug>/` (HTML screens + clickable index gallery) with the shared screenshot contract (`?t=`/`?state=`/`?device=`/`?level=` frozen frames), captures via `scripts/app-shots.mjs`, iterates, then graduates winners to a real prototype app. Two tiers: fast hand-rolled HTML (default — motion/interaction/layout) or `--ui` = real Nuxt UI v4 in Vue+Vite (`mockups/ui-proof` is the proven starter). `mockups/` is outside the workspace. Taxonomy: mockup (fake/clickable) → prototype (real/running, `pocs/`→`prototypes/` #866) (#863) |
