...
- **Rung 2 — merge→`main`, on the deployed `*.pmcp.dev` (integration):** path-filtered staging deploy with remote D1 migrations, deployed smoke against the live URL, `report-failed-deploy.yml` opens an issue on a red post-merge deploy, nightly full-matrix e2e backstop. Being hardened in #1106 (a report-only smoke failure currently leaves the run green).
- **Rung 3 — before prod (confidence gate):** production is a deliberate manual `workflow_dispatch` only (#318), via the `/deploy-production` skill's pre-flight ("verified on staging first"). CI enforcement — refuse to deploy prod while the staging smoke is red — is #1107.
