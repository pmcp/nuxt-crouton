...
| Skill | `.claude/skills/prototype/SKILL.md` | Explore an idea as throwaway, **clickable** no-build prototypes *before* a POC — the lighter, earlier sibling of `/ui-proposal`. Scaffolds `prototypes/<slug>/` (HTML screens + clickable index gallery) with the shared screenshot contract (`?t=`/`?state=`/`?device=` frozen frames), captures via `scripts/app-shots.mjs`, iterates, then graduates winners to `pocs/`. Two tiers: fast hand-rolled HTML (default — motion/interaction/layout) or `--ui` = real Nuxt UI v4 in Vue+Vite (`prototypes/ui-proof` is the proven starter). `prototypes/` is outside the workspace (#863) |
