...
| Skill | `.claude/skills/graduate/SKILL.md` | The deliberate **"step back and rebuild"** checkpoint after heavy POC iteration: turn a proven `pocs/*` app into a real, **test-first**, documented **package (or packages) + a consuming app**. Starts with a handoff brief (what it does · features · gotchas → the test spec), then rebuilds test-first (the #774 gate fires on `packages/*`), emits app + package(s), documents, and promotes (`/deploy`). The POC removes the "we don't know what it should do" TDD objection; the package is the durable value (opinionated ↔ ecosystem-grade). Use when a POC is "done" / "graduate this poc" / "make it a real app" (#916) |
