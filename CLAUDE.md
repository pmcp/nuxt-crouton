...
**Agent-posted GitHub comments LEAD with a 🤖 provenance header.** A comment written by an agent via the GitHub tools (`add_issue_comment`) posts under the **@pmcp** account — not as a bot — so it reads like Maarten wrote it. Every such comment MUST start its body with a first line that names the source + context, e.g. `> 🤖 **Claude Code** · interactive agent, posted from @pmcp's account (not Maarten) · _<one-line context>_` (use `agent pipeline (CI)` for the autonomous flow). The auto-appended "Generated by Claude Code" footer is too easy to miss; the header is the unmistakable signal. Enforced by the `require-comment-provenance` PreToolUse hook (`.claude/hooks/`), which blocks a comment lacking the `🤖` header. (Applies to comments only — issue/PR *bodies* are already clearly agent-authored work items.)
