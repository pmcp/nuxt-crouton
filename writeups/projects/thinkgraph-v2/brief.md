...
## Status as of 2026-04-10 (re-audited)
...
> **Re-audit note (2026-04-10):** Significant work shipped between 2026-04-08 and 2026-04-10 that was NOT in any original brief: a Notion-style block editor with Pi collaboration (comments, file diffs, action buttons). This section now reflects that capability. Previous audit: 2026-04-07 PM pass found phases had quietly progressed (commits `4be788b2`, `4bccb0e3`, `b10beff6`, `22f6f452`, `a98189bc`).

### Convergence brief (`thinkgraph-convergence-brief.md`)

| Phase | Status | Notes |
|-------|--------|-------|
| Phase 1A — MDC rendering | 🟡 **PARTIAL** | `@nuxtjs/mdc` is installed (`apps/thinkgraph/package.json:19`) and `NodeDetail.vue:354,385` renders `node.brief`/`node.output` via `<MDC>`. **Gaps:** no custom ThinkGraph MDC components built (`::chart`, `::data-table`, etc. — no `app/components/mdc/` dir exists). **Update 2026-04-10:** the 15 dormant dispatch services that emitted non-MDC artifacts (code, mermaid, etc.) have been removed — only `pi-agent` remains. MDC gap is now purely about custom components, not service output format. |
| Phase 1B — Graph validation (`thinkgraph check`) | ✅ **SHIPPED** | `apps/thinkgraph/server/utils/validate-graph.ts` validates `broken-context-ref`, `broken-depends-on`, `orphan-node`, `duplicate-title-at-depth`, `stuck-active`, `broken-wiki-link`, `stuck-worker` (commit `4be788b2`), and **cycles, disconnects, unknown steps** (commit `ee52ed7c`). MCP tool `apps/thinkgraph/server/mcp/tools/check-graph.ts` and API endpoint `apps/thinkgraph/server/api/teams/[id]/thinkgraph-nodes/validate.get.ts` both wired. **Gaps:** no canvas visual indicators on invalid nodes; no scheduled Nitro task (both marked optional in original spec). |
...
| Step 3 — Canvas presence | ✅ **SHIPPED** (was 🟡) | Pi worker infra (`apps/thinkgraph-worker/src/yjs-client.ts` ~500 lines, `yjs-pool.ts` ~188 lines, 14 `appendAgentLog`/`setAgentStatus` calls in `session-manager.ts`) was already shipped — the gap was purely the rendering layer, now wired against the new `node.ephemeral` namespace (see Option C entry below). `ThinkgraphNodesNode.vue` renders a pulsing Pi status pill in the card header + a live latest-log-line below the existing activity text, reading `props.data.ephemeral.agentStatus` / `props.data.ephemeral.agentLog` (the bridge surfaces Yjs `node.ephemeral` as `data.ephemeral` because Vue Flow only forwards the `data` prop). New `apps/thinkgraph/app/components/NodeAgentActivity.vue` renders a full scrollable log feed with auto-scroll-on-new-entry (via `nextTick`, no `setTimeout`) inside the project-page slideover (`apps/thinkgraph/app/pages/admin/[team]/project/[projectId].vue`), conditional on `flowId && selectedItemId`. The drawer feed uses its own `useFlowSync({ flowId, collection: 'thinkgraphNodes' })` so the WS lifecycle is tied to slideover mount — known cost: a second WebSocket to the same flow room while the drawer is open. Decision #5 (where to render): **option C — both card pill + drawer feed**. Live-verified end-to-end on 2026-04-08 once the Pi worker was actually running the new build (`91e61371` + `17ce3122` had been local-only for a day before being pushed): pulsing pill renders on the card while Pi is working, "Agent activity" section appears in the slideover with status indicator + scrollable log feed + auto-scroll. **Quality bugs all fixed:** text duplication + thinking fragmentation resolved via `upsertAgentLog` with stable `streamKey` per `(messageId, slot)` — overwrites instead of appending on each `message_update`. `tool_result` raw JSON resolved via `extractToolResultText()` helper. All in `session-manager.ts` + `yjs-client.ts`. |
| Step 4 — Background reflection | ⛔ **NOT STARTED** | No reflection / background synthesis task. Depends on Step 2 being fully built. |

### Block editor + Pi collaboration (emerged 2026-04-08 → 2026-04-10, not in original briefs)

This is the most significant new capability since the v2 audit. It moves ThinkGraph from "dispatch and wait" to "collaborate in real-time inside each node."

| Component | File | What it does |
|-----------|------|-------------|
| Block editor | `apps/thinkgraph/app/components/NodeBlockEditor.vue` | Tiptap-based Notion-style editor embedded in node slideover. Slash menu, collab connection indicator, stores content in `node.content` (migration `0017_add_node_content.sql`). |
| Action buttons | `apps/thinkgraph/app/components/Blocks/ActionButtonBlock.vue` + `app/extensions/action-button.ts` + `app/composables/useNodeActionHandlers.ts` | Pi inserts interactive buttons into the editor. User clicks trigger handlers. |
| File diffs | `apps/thinkgraph/app/components/Blocks/FileDiffBlock.vue` + `app/extensions/file-diff.ts` + `apps/thinkgraph-worker/src/file-diff-tools.ts` | Pi proposes file changes as inline diffs the user can review. |
| Comment threads | `apps/thinkgraph/app/components/CommentSlideout.vue` + `app/composables/useNodeComments.ts` + `app/extensions/comment-anchor.ts` | Text-selection comment composer. Passages anchor comments to editor content. |
| Pi comment tools | `apps/thinkgraph-worker/src/comment-tools.ts` | Pi can open comment threads and reply to existing ones via Yjs. |
| "Ask Pi" reply | `apps/thinkgraph/server/api/teams/[id]/dispatch/comment-reply.post.ts` | Dispatches Pi to reply to a comment thread. |
| Pi page writing | `apps/thinkgraph-worker/src/page-tools.ts` + `yjs-page-client.ts` | `append_block` tool lets Pi write blocks into the editor. Parses markdown. |
| Comment → node | `CommentSlideout.vue` (commit `cdd60637`) | Promote comment threads to child nodes. |
| Analyst comments | `session-manager.ts` (commit `c6fdcbe7`) | Analyst stage emits passage-level findings as comments instead of just returning a verdict. |
| Node suggestions | `pi-extension.ts` (commit `e51cf01c`) | `create_node` MCP tool replaced with comment-based suggestions. |

**Implications:**
- The node slideover is now the primary collaboration surface, not just a detail view.
- Pi's analyst stage produces richer, anchored feedback (comments on specific passages) rather than monolithic verdicts.
- The `create_node` → comment-suggestion shift means new child nodes require user approval via the comment UI rather than being created directly.
- File upload on nodes also shipped (`useNodeFileUpload.ts`, commit `6036bed9`).

### Open cleanup debt (audit findings, 2026-04-10 re-audit)

**Cleared since the 2026-04-07 audit:**
- ✅ Legacy `decisions/`/`workitems/` collections + 11 dead endpoints + `ThinkgraphWorkitemsNode.vue` deleted (commit `b10beff6`).
- ✅ `research-agent.ts` removed from dispatch services (commits `22f6f452`, `a98189bc`).
- ✅ **15 dormant dispatch services removed** (commit `ddcc4aa7`). Only `pi-agent.ts` remains in `apps/thinkgraph/server/utils/dispatch-services/`.
- ✅ **Conversation now reaches Pi.** `dispatch-conversation.ts` produces AI-generated roundup cached on `chatconversations.metadata.dispatchSummary`. Wired into `work-item.post.ts`.
- ✅ **Canvas agent activity now visible.** Step 3 rendering layer shipped. `data`/`ephemeral` split (Option C) shipped.
- ✅ **Agent log streaming bugs partially fixed.** `tool_result` truncation fixed (commit `63336136`). Streaming cleanup (commit `80563fcd`).
- ✅ **Graph validation expanded** with cycles, disconnects, unknown steps (commit `ee52ed7c`).
- ✅ **`analyse` stage routing fixed** — now routes to `discoverInstructions` (commit `f3096210`).
- ✅ **Ancestor worktree surfacing** in dispatch context chain (commit `feebce9e`).
- ✅ **Merger worktree check** — checks current worktree field before falling back to ancestors (commit `4bac146e`).

**Still open:**

- ~~**Agent activity log quality bugs.**~~ ✅ **All three fixed.** (1) Text duplication fixed via `upsertAgentLog` with stable `streamKey` per `(messageId, slot)` — overwrites instead of appending on each `message_update`; (2) `tool_result` raw JSON fixed via `extractToolResultText()` helper; (3) Thinking fragmentation fixed via same `upsertAgentLog` pattern for thinking blocks. All in `session-manager.ts` + `yjs-client.ts`.
- ~~**Project page slideover doesn't reactively refresh after dispatch completes.**~~ ✅ **Fixed.** 3-second polling while any item has `active`/`working` status (lines 157-171 of `[projectId].vue`), plus `crouton:mutation` and `crouton:remoteChange` hooks. `selectedItem` is a computed from `items.value` so it updates automatically when the poll fetches new data.
- **`stuck-worker` validator has no scheduled runner.** Detection logic exists; nothing calls it on a schedule.
- **Wiki-link resolution not wired to save.** `resolveWikiLinksForNode()` exists but no node mutation calls it.
- **`sync-changelogs` parallel to Phase 2A runner.** Both legacy and new systems active. Pick one.
- **Legacy DB tables `thinkgraph_decisions`/`thinkgraph_workitems` not dropped.** Code references all gone; tables still in D1.
- **Migration filename collisions.** `0004_first_ironclad.sql` and `0009_add_user_role.sql` orphans still on disk. See [implementation-notes.md](implementation-notes.md#migration-filename-collisions-2026-04-07).
- **Worker→app delivery has no durability layer.** No outbox / retry / idempotency on the Pi worker.
